...
const buildStandardizedPayload = (formId, quizVersion, leadData, stepData, currentItem, items, options = {}) => {
  ...
  return {
    ...
    // --- CONTATO (dados do formulário final) ---
    contact: {
      nome,
      whatsapp,
      email,
    ...
    // --- ITENS ADICIONAIS (outras persianas/cortinas) ---
    itens_adicionais,
    itens_adicionais_count: itens_adicionais?.length ?? 0,

    // --- JORNADA (etapas percorridas, para análise) ---
    journey: {
      steps_completed: stepsHistory,
      steps_count: stepsHistory?.length ?? 0
    },

    // --- DADOS BRUTOS (fallback para integrações que usam chaves planas) ---
    _flat: {
      form_id: formId || '',
      quiz_version: quizVersion || '',
      submitted_at: submittedAtISO,
