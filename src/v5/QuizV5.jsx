...
const WEBHOOK_QUIZ_V5_URL = import.meta.env.VITE_WEBHOOK_QUIZ_V5_URL || 'https://n8n.sitespdoze.com.br/webhook/quizv5';
...
function getProdutoFromItemV5(item) {
...
export default function QuizV5() {
  ...
  const optionsPasso2 = useMemo(() => {
    ...
    if (!ambiente) return [];
    const opts = getModelosParaV5(ambiente).map((m) => ({ ...m, nextStep: 'passo_3_tecido' }));
  ...
  const optionsPasso3 = useMemo(() => {
    ...
    const tecidos = getTecidosParaV5(ambiente, modelo);
  ...
  const modifiedStep = useMemo(() => {
    ...
    if (activeStep.id === 'passo_7_observacoes') step.inputs = [{ id: 'observacoes', label: '', placeholder: 'Ex: outro ambiente, preferências...', type: 'textarea' }];
    return step;
  ...
  const handleNext = (stepData) => {
    ...
    if (activeStep.isFinal) {
      ...
      const payload = buildPayloadV5(formId, leadData, stepData, updatedCurrentItem, { sessionStartedAt, stepsHistory });
      const webhookPayload = { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) };
      const promises = [];
      const webhookLabels = [];
      if (WEBHOOK_QUIZ_V5_URL) {
        promises.push(fetch(WEBHOOK_QUIZ_V5_URL, webhookPayload));
    ...
    if (activeStep.id === 'passo_6_medidas' && stepData?.passo_6_medidas) nextStepId = 'passo_7_observacoes';
    if (activeStep.id === 'passo_7_observacoes') nextStepId = 'passo_8_captura';
